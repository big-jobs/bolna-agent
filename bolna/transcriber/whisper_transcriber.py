...
class WhisperTranscriber(BaseTranscriber):
    def __init__(self, telephony_provider, input_queue=None, model='whisper', stream=True, language="en",
                 endpointing="400",
        ...
        self.segments_list: Optional[list] = None
        ...
        async for msg in ws:
            try:
                ...
                if "message" in msg and msg["message"] == "UTTERANCE_END":
                    logger.info(f"segment list{self.segments_list}")
                    ...
                    self.meta_info["utterance_end"] = self.connection_start_time + float(
                        self.whole_segment_list[-1].get('end'))
                ...
                if self.process_interim_results:
                    ...
                    if "segments" in msg and self.seg_ptr > -1:
                        if self.seg_ptr != self.current_seg_ptr:
                            self.finalized_transcript += " " + self.segments_list[self.seg_ptr].get(
                                "text")  # Just get the whole transcript as there's mismatch at times
